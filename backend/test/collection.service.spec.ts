...
describe('CollectionService', () => {
  ...
  describe('createCollection', () => {
    it('should create a collection', async () => {
      ...
      expect(result.token_id).toBe(mockTokenId);
      expect(result.name).toBe(createCollectionDto.name);
      expect(result.symbol).toBe(createCollectionDto.symbol);
      expect(result.metadata).toBe(createCollectionDto.description);
      expect(result.created_timestamp).toBeInstanceOf(Date);
    });
  });
